...
## ライブラリの更新と開発モード (Development Mode)

COHERENTのコア（プロダクト）は頻繁にアップデートされます。
開発中の最新の変更を即座に反映させるには、**Editable Install (編集可能インストール)** を利用してください。
...
*   **仕組み**: ライブラリのソースコードへのシンボリックリンクが作成されます。
*   **メリット**: `coherent/` ディレクトリ内のコードを変更（git pull等）すると、再インストール不要で即座にアプリケーションに反映されます。

### 本番環境・CI/CDでの更新戦略

本番環境など、安定性が求められる環境では以下のように運用してください：

1.  **Git依存インストール**: 特定のコミットハッシュやタグを指定してインストールします。
    ```bash
    pip install git+https://github.com/your-org/COHERENT.git@v0.1.0
    ```
2.  **定期アップデート**: `pip install --upgrade .` またはCIパイプラインで定期的に再ビルド・デプロイを行います。
